...
tabla <- datos %>%
  filter(!is.na(area_geo), !is.na(asis_med)) %>%
  count(area_geo, asis_med) %>%
  group_by(area_geo) %>%
  mutate(prop = n / sum(n)) %>%
  ungroup()

tabla %>%
  arrange(area_geo, desc(n)) %>%
...
datos %>%
  ...
  group_by(area_geo) %>%
  mutate(prop = n / sum(n)) %>%
  ggplot(aes(x = area_geo, y = prop, fill = asis_med)) +
...
```{r p3_sexo_lugar}
datos %>%
  filter(!is.na(sexo), !is.na(lugar_ocu)) %>%
  count(sexo, lugar_ocu) %>%
  group_by(sexo) %>%
  mutate(prop = n / sum(n)) %>%
  ungroup() %>%
...
# Selecciona variables comunes (ajusta nombres si cambian en tu dataset)
df_clust <- datos %>%
  transmute(
    edad_anios = edad_anios,
    sexo = as.factor(sexo),
    area_geo = as.factor(area_geo),
    asis_med = as.factor(asis_med),
    lugar_ocu = as.factor(lugar_ocu)
  ) %>%
  drop_na()

# Distancia de Gower
d_gower <- daisy(df_clust, metric = "gower")
```

## Elegir número de clusters (k) con silueta

```{r clustering_silueta}
set.seed(123)
k_vals <- 2:6
sil_avg <- map_dbl(k_vals, ~{
  pam_fit <- pam(d_gower, k = .x)
  mean(pam_fit$silinfo$widths[, "sil_width"])
})

data.frame(k = k_vals, silueta_promedio = sil_avg) %>%
  kable(caption = "Silueta promedio por k (PAM + Gower)") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE)
```

```{r clustering_silueta_plot, fig.width=9}
data.frame(k = k_vals, silueta_promedio = sil_avg) %>%
  ggplot(aes(x = k, y = silueta_promedio)) +
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
  labs(title = "Selección de k por silueta", x = "k (número de clusters)", y = "Silueta promedio")
```

## Ajuste final y perfil de clusters

```{r clustering_final}
k_opt <- k_vals[which.max(sil_avg)]
set.seed(123)
pam_final <- pam(d_gower, k = k_opt)
...
res_num %>%
  kable(caption = "Resumen por cluster (edad)") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE)
...
top_cat("sexo") %>% kable(caption="Top 3 sexo por cluster") %>% kable_styling(full_width=FALSE)
top_cat("area_geo") %>% kable(caption="Top 3 área por cluster") %>% kable_styling(full_width=FALSE)
top_cat("asis_med") %>% kable(caption="Top 3 asistencia médica por cluster") %>% kable_styling(full_width=FALSE)
top_cat("lugar_ocu") %>% kable(caption="Top 3 lugar de ocurrencia por cluster") %>% kable_styling(full_width=FALSE)
```
